...
def probabilidadesParaDibujar(Theta1, Theta2, X):
    ...
    capa_oculta = sigmoide(input)
    capa_oculta = np.append(np.ones(shape=(1,capa_oculta.shape[1])),capa_oculta,axis=0)
    proba = sigmoide(Theta2*capa_oculta)
...
def backProp(p, numero_de_entradas, capas_ocultas, numero_de_etiquetas, X, valor_de_y, l=0.2):
    """Backpropagation algorithm of neural network"""
    
    Theta1 = np.reshape(p[:capas_ocultas*(numero_de_entradas+1)], (capas_ocultas,-1))
    Theta2 = np.reshape(p[capas_ocultas*(numero_de_entradas+1):], (numero_de_etiquetas,-1))
    m = len(X)
    delta1 = 0
    delta2 = 0
    print(range(m))
    for t in range(m):
        ...
        z2 = Theta1*a1
        a2 = np.append(np.ones(shape=(1,z2.shape[1])), sigmoide(z2),axis=0)
        z3 = Theta2*a2
        a3 = sigmoide(z3)
...
def J(theta, numero_de_entradas, capas_ocultas, numero_de_etiquetas,X, valor_de_y, l=0.2):
    """Cost funtion"""
    
    Theta1 = np.reshape(theta[:capas_ocultas*(numero_de_entradas+1)], (capas_ocultas,-1))
    Theta2 = np.reshape(theta[capas_ocultas*(numero_de_entradas+1):], (numero_de_etiquetas,-1))
    m = len(X)
    X = np.append(np.ones(shape=(X.shape[0],1)),X,axis=1)
    J = 0
    for i in range(m):
        ...
        hx = sigmoide(Theta2*np.append([[1]], sigmoide(Theta1*x.transpose()), axis=0))
